...
const validationUpdateCompany = Yup.object({
  ...
  taxCode: Yup.number()
    .typeError('Mã số thuế phải là số nguyên')
    .required('Mã số thuế là bắt buộc')
    .test('length', 'Mã số thuế phải có từ 10 đến 15 chữ số', value => value && value.toString().length >= 10 && value.toString().length <= 15),
