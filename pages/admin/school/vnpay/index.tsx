...
import { useGetAllWalletsQuery, useGetAllWorShopsUniversityQuery } from '@/services/adminSchoolApi';
type ItemType =
...
const Wallet = () => {
  ...
  const items: ItemType[] = [
    {
      label: (
        <Link href={'#!'} className="min-w-[150px] text-base">
          Thêm ví mới
        </Link>
      ),
      key: '0',
                ...
                <table className="w-full table-auto">
                  ...
                  <tbody>
                    {workshops?.data?.content.length > 0 ? (
                      workshops?.data.content.map((item, index) => (
                        <tr key={item.id} className={index % 2 !== 0 ? 'bg-[#F7F6FE]' : 'bg-primary-white'}>
                          <td className="px-5 py-4"> {index + 1 + (page - 1) * size}</td>
                          <td className="px-5 py-4">{item.workshopTitle}</td>
                          <td className="px-2 py-4">
                            <p className="sm:[230px] w-[210px]">{item.university.universityName}</p>
                          </td>
                          <td className="px-5 py-4">{item.startTime}</td>
                          <td className="px-5 py-4">
                            <Chip
                              label={statusTextWorkshop(item.moderationStatus).title}
                              style={{
                                color: `${statusTextWorkshop(item.moderationStatus).color}`,
                                background: `${statusTextWorkshop(item.moderationStatus).bg}`,
                              }}
                            />
                          </td>
                        </tr>
          ...
          <div className="flex w-1/3 max-w-full flex-col gap-8 px-3">
            <div className="single-items rounded-lg bg-white p-[30px]">
              <div className="card-head mb-8 flex items-center justify-between">
                ...
                <div className="cursor-pointer rounded-full p-2 text-sm font-semibold text-[#475467] hover:bg-slate-200">
                  <Dropdown
